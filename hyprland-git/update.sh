...
git push && \
hyprlandGitBuildId=$(curl "${curl_opts[@]}" "https://copr.fedorainfracloud.org/webhooks/custom/233782/${COPR_WEBHOOK}/hyprland-git") && \
copr watch-build "${hyprlandGitBuildId}" && \
curl "${curl_opts[@]}" "https://copr.fedorainfracloud.org/webhooks/custom/233782/${COPR_WEBHOOK}/hyprland-plugins-git"; }

if [[ $newRelease == "1" ]]; then
    hyprlandBuildId=$(curl "${curl_opts[@]}" "https://copr.fedorainfracloud.org/webhooks/custom/233782/${COPR_WEBHOOK}/hyprland")
    copr watch-build "${hyprlandBuildId}"
    curl "${curl_opts[@]}" "https://copr.fedorainfracloud.org/webhooks/custom/233782/${COPR_WEBHOOK}/hyprland-plugins"
    git branch "$newTag"
